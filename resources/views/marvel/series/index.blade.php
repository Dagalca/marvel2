...
@section('content')
        ...
        <div
            ...
            <div class="flex h-full items-center justify-center">
                <div class="px-6 text-center text-white md:px-12">
                    ...
                    <p class="text-xl mt-2 mb-8">Explora las series de Marvel, donde cada episodio es una puerta a mundos extraordinarios y enfrentamientos épicos que definen a los héroes.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('series.index') }}" method="GET" class="mb-6">
        <div class="flex items-center justify-center mt-5 mb-5">
            <input type="text" name="search" placeholder="Buscar series..." class="text-xl w-50 px-4 py-2 border-b-2 border-white focus:outline-none focus:border-red-600 bg-transparent placeholder-white text-white">
    ...
    <div class="container mx-auto px-4">
                ...
                <div class="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 text-white flex flex-col relative">
                    ...
                    @auth
                        <form action="{{ route('favorites.add') }}" method="POST">
                            @csrf
                            <input type="hidden" name="favoritable_id" value="{{ $serie['id'] }}">
                            <input type="hidden" name="favoritable_type" value="series">
                            <input type="hidden" name="title" value="{{ $serie['title'] }}">
                            <input type="hidden" name="image_url" value="{{ $serie['thumbnail']['path'].'.'.$serie['thumbnail']['extension'] }}">
                            <input type="hidden" name="description" value="{{ $serie['description'] }}">
                    ...
                    <div class="p-4 flex-grow">
                        ...
                        <p class="text-gray-300 mt-2">Creadores: {{ $serie['creators']['items'] ? implode(', ', array_column($serie['creators']['items'], 'name')) : 'Creadores no disponibles' }}</p>
                    </div>
        ...
        {{-- Paginación --}}
        @if(is_object($series) && method_exists($series, 'links'))
            <div class="mt-4">
                {{ $series->links() }}
            </div>
        @endif
